...
class Diagnosis:
    ...
    def __init__(self) -> None:
        self.model = TCN(output_size=4, num_channels=[40] * 8, kernel_size=6, dropout=0.05)
        self.model.build(input_shape=([1, 800]))
        self.model.load_weights("./checkpoints/TCN_custom_800_986.h5")
    ...
    def diagnosis(self, data_path):
        ...
        predicted = [len(np.where(label_pre == 0)[0]), len(np.where(label_pre == 1)[0]), len(np.where(label_pre == 2)[0]), len(np.where(label_pre == 3)[0])]
        print("[WARN] Predicted rate(N-M-U-B):", predicted)
        result = predicted.index(max(predicted))

        if max(predicted) < len(label_pre) // 2 + 5: return "Normal"
